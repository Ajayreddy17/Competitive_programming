// requires Bit_vector, Monoid_Add
...
struct Wavelet_Matrix {
    ...
    pair<T, X> kth_value_and_sum(int L, int R, int k, T xor_val = 0) {
        ...
        assert(0 <= k && k <= R - L);
        if (k == R - L) { return {inf<T>, sum_all(L, R)}; }
        int cnt = 0;
        X sm = MX::unit();
        T ret = 0;
    ...
    // xor した結果で、[L, R) の中で k>=0 番目と prefix sum
    pair<T, X> kth_value_and_sum(vector<pair<int, int>> segments, int k,
                                 T xor_val = 0) {
        assert(!cumsum.empty());
        if (xor_val != 0) assert(set_log);
        int total_len = 0;
        ...
        if (k == total_len) { return {inf<T>, sum_all(segments)}; }
        int cnt = 0;
        X sm = MX::unit();
        T ret = 0;
        for (int d = lg - 1; d >= 0; --d) {
