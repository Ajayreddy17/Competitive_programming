...
struct fenwick_tree_2d_sparse_sum{
    ...
    fenwick_tree_2d_sparse_sum(vector<pair<array<B, 2>, T>> init): X(init.size()){
        ...
        for(auto i = 0; i < (int)init.size(); ++ i){
            X[i] = init[i].first[0];
        }
        sort(X.begin(), X.end());
    ...
    T query(B _xl, B _xr, B _yl, B _yr) const{
        return pref(_xr, _yr) - pref(_xl, _yr) - pref(_xr, _yl) + pref(_xl, _yl);
    }
    T query(B _x, B _y) const{
        return pref(_x + 1, _y + 1) - pref(_x + 1, _y) - pref(_x, _y + 1) + pref(_x, _y);
    }
};
